...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done, player):
    ...
    def learn(self, experiences, gamma, player):
        ...
        Q_targets_next = self.critic_target(next_states, actions_next, player)
        # Compute Q targets for current states (y_i)
        Q_targets = rewards + (gamma * Q_targets_next * (1 - dones))
        # Compute critic loss
        Q_expected = self.critic_local(states, actions, player)
        ...
        actor_loss = -self.critic_local(states, actions_pred, player).mean()
        # Minimize the loss
